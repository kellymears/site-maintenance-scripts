...
syncScheduler = {
  ...
  runCron: function() {
    for (let site of this.config.sites) {
      const localEnabled = site.local ? '--local' : null,
        ...
        cmd = `./${site.script} ${site.origin} ${site.destination} ${localEnabled}`

      cron.validate(site.schedule)
        ? cron.schedule(site.schedule, () => {
            this
              .consoleOut.siteHeader(site)
              .runSync(path, cmd)
  ...
  consoleOut: {
    ...
    invalidCronFormat: function(site) {
      shell.echo(`Cron schedule "${ site.schedule}" is invalid. Please check your configuration.`)
      return this
    },
  },
}
 ...
 * Export module
 *
 * @exports object syncScheduler
 */
module.exports = { syncScheduler }

/**
 * Or, run script directly.
 * (remove this if you are exporting)
 */
syncScheduler
  .init(config)
  .validateSSH()
